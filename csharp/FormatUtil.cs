...
namespace Nova {
	...
	public static class FormatUtil {
		...
		/// <summary>
		/// Converts a vector to a string with a fixed width string representation.
		/// </summary>
		public static string ToFixedString(this Vector2 vector) => string.Format("({0,9:f3}, {1,9:f3})", vector.x, vector.y);

		/// <summary>
		/// Converts a vector to a string with a fixed width string representation.
		/// </summary>
		public static string ToFixedString(this Vector2 vector, int precision) {
			return string.Format("({0," + (precision + 6) + ":f" + precision + "}, {1," + (precision + 6) + ":f" + precision + "})",
				vector.x, vector.y);
		}
